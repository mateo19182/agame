...
import { writeName, writeHostMode } from "@/lib/roomSession";
    ...
    <main className="flex-1 flex flex-col items-center justify-center px-6 py-12">
      <div className="w-full max-w-3xl text-center">
        ...
        <p className="mt-4 text-[color:var(--muted)] text-lg max-w-xl mx-auto">
          A couch game for two. Trivia duels and quick minigames — pick your mix, then play on the big screen with two
          phones.
        </p>
      </div>
      ...
      <div className="mt-12 grid w-full max-w-3xl gap-5 sm:grid-cols-2">
        <div className="glass rounded-3xl p-6 sm:p-8 flex flex-col">
          ...
          <p className="mt-2 text-sm text-[color:var(--muted)]">You&apos;ll get a room code. Players join with their phones.</p>
          <button
            type="button"
            onClick={() => setHostPlays((v) => !v)}
            className="mt-6 w-full flex items-center justify-between px-4 py-3 rounded-2xl border border-white/10 text-left"
          >
            <span>
              <span className="font-semibold block">{hostPlays ? "I'll play on this screen" : "Scoreboard only"}</span>
              <span className="text-xs text-[color:var(--muted)]">
                {hostPlays ? "Big screen is player 1 — one phone joins" : "Big screen just shows scores — two phones join"}
              </span>
            </span>
            <span className={`shrink-0 w-12 h-7 rounded-full p-1 transition ${hostPlays ? "bg-[color:var(--accent)]" : "bg-white/10"}`}>
              <span className={`block w-5 h-5 rounded-full bg-white transition ${hostPlays ? "translate-x-5" : ""}`} />
            </span>
          </button>
          {hostPlays && (
            <input
              ...
              maxLength={16}
              className="mt-3 px-4 py-3 rounded-2xl bg-black/30 border border-white/10 outline-none focus:border-[color:var(--accent)]"
            />
          )}
          <button
            onClick={startHost}
      ...
      <div className="mt-10 text-center text-sm text-[color:var(--muted)]">~6 minute rounds · 2 players · pure vibes</div>

      <div className="mt-6 flex items-center gap-5">
        <Link to="/settings" className="text-sm text-[color:var(--muted)] hover:text-white underline underline-offset-4">
          Settings
        </Link>
        <LogoutButton />
      </div>
